...
{
    if (v.empty()) throw runtime_error("random_element: empty vector");
    static random_device rd;
    static mt19937 gen(rd()); // static to avoid reseeding every call
...
{"Mew","100","100","Psychic","Transform","Metronome","SoftBoiled"}
    ...
    const auto& entry = random_element(pokedex);
    for (int i = 0; i < 4; ++i) {
        cout << "Name: " << entry[0] << ", HP: " << entry[1] << ", Attack: " << entry[2]
            << ", Move 1: " << entry[3] << ", Move 2: " << entry[4]
            << ", Move 3: " << entry[5] << ", Move 4: " << entry[6] << endl;
	}
